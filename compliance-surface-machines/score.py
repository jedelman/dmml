#!/usr/bin/env python3
"""Run results/dispatch.ndjson through the real Surface-syntax machine
parser (dmml-hs/app/ComplianceCheckSurface.hs, routed to
DMML.Surface.parseMachineSurface via each record's "kind": "machine")
...
def main() -> int:
    ...
    lines = ["# DMML Surface-syntax MACHINE authoring-compliance checkpoint", ""]
    lines.append(f"{len(verdicts)} (model, scenario) dispatches scored against the real "
                  "`DMML.Surface.parseMachineSurface` production parser.")
    lines.append("")
