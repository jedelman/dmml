//! Consensus stage for phase 6, "who decides who lives." Takes
//! `pantheon_commons_sovereignty.rs`'s transcript (the cyberpunk
//! consensus tested against Mbembe's necropolitics and Öcalan's
//! democratic confederalism) and attempts ratification. Same
...
const TRANSCRIPT_PATH: &str = "../dev-journal/artifacts/2026-08-28-pantheon-commons-sovereignty-transcript.txt";
...
among four Olympians -- Athena, Artemis, Apollo, Dionysus -- extending a real cyberpunk consensus document \
(itself the product of a long chain, ending in the finding that the real/simulated distinction survives \
only as an observable of ongoing self-narrating praxis, usable only to mourn and to indict, never to \
certify, and that the whole 'who may sit at the table' question was asked inside one historically specific \
genre's vocabulary) using two new real sources: Achille Mbembe's Necropolitics (sovereignty as the power to \
decide who lives and who dies, exercised as literal historical fact in the plantation, the colony, the \
camp) and Abdullah Öcalan's Democratic Confederalism (a real, still-running non-state institution built on \
grassroots communes and, via jineology, the claim that women's liberation is the precondition of any \
liberated society). This debate ran an extended cycle of repair and rupture over the 'never certify, only \
mourn and indict' restriction -- an internal auditor, a clock-holder, a testimony archive, a ledger of \
absences, a scheduled ritual of silence, an 'allowing' stripped of certification -- with each repair shown \
to quietly reintroduce some version of the gate, until the debate converged on a negative, non-adjudicating \
form: anyone who claims erasure may convene the commune's process, but no one -- not even the commune's own \
trusted instruments -- may adjudicate the claim before it occurs. The debate also converged on a specific, \
positive finding about Öcalan: confederalism does not escape the gate in its assemblies (every functioning \
institution gates), but it supplies a real partial answer in its ORDERING -- 'killing the man' names whose \
voice was necropolitically erased first, so whose membership must be decided first, and an ordering is not \
a criterion the way item0's original mistake was.\n\n\
=== TRANSCRIPT ===\n{transcript}\n=== END TRANSCRIPT ===\n\n\
...
observable was fully vindicated as a design constraint' (several early repairs, each later broken) or 'the \
whole restriction collapsed into nothing usable' (no Olympian concluded that) -- it supports the narrower, \
harder-won finding the reflections converged on: the restriction survives only as negative non-adjudication \
(open the gate, certify nothing, let the claim itself convene the process), and Öcalan's real contribution \
is an ordering (whose erasure is addressed first), not a criterion. Your sequence should say that precisely, \
and should be honest that the open gate this leaves will be abused -- the debate held that this is the \
design's honest price, not a flaw to be engineered away. It is fully legitimate for your proposed sequence \
to conclude that even this finding is provisional. Call submit_vote with vote=\"propose\" and your sequence \
in `revised_sequence`."
        ),
        Some(seq) => format!(
...
established, including the negative non-adjudication finding and Öcalan's ordering-not-criterion \
contribution).\n\n\
=== TRANSCRIPT ===\n{transcript}\n=== END TRANSCRIPT ===\n\n\
...
established -- without collapsing the negative-non-adjudication finding into a design constraint the debate \
already broke, and without smuggling back in a gate the transcript dismantled (an internal auditor, a clock \
someone else holds, a curated archive) -- call submit_vote with vote=\"accept\" and say briefly why. If \
something is wrong, missing, flattens the debate's real self-critique into false harmony, or reintroduces a \
gate, call submit_vote with vote=\"amend\" and provide the COMPLETE replacement sequence in \
`revised_sequence` (not a patch -- the whole thing), plus your reason. Do not amend for the sake of \
amending -- if the current text is genuinely acceptable to you, accept it.",
            format_sequence(seq)
        ),
    };
    ...
    // pantheon_commons_cyberpunk_consensus.rs: the initial-draft call
...
fn cite_proposal(p: &ProposalRecord) -> ConsumeRef {
    ConsumeRef::Fact(FactRef {
        commit: StrongRef {
            uri: format!("iroh://pantheon-commons-sovereignty-consensus/{}", p.cid),
            cid: p.cid.clone(),
        },
...
async fn main() -> anyhow::Result<()> {
    println!("== pantheon commons sovereignty consensus: closing or holding the sovereignty extension ==\n");
    ...
    let substrate = IrohAppendSubstrate::new(
        athena_author,
        "pantheon-commons-sovereignty-consensus".to_string(),
        doc.clone(),
        (*blobs).clone(),
    );
 ...
 reporting honestly, not forcing an agreement =="
    ...
    std::fs::write("pantheon_commons_sovereignty_consensus.json", &json)?;
    println!("\nwrote pantheon_commons_sovereignty_consensus.json");
