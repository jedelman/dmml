...
async fn explain(
    ...
    let user_msg = format!(
        "Below is a synthesis your group (four Olympians) reached after a long debate about three \
...
the commons and the body), and Elinor Ostrom's Governing the Commons (the eight design principles for \
durable commons). Your group coined its own vocabulary mid-debate -- terms like \"the zoo-wolf,\" \
\"minting rapture,\" \"the wild feast as sense-organ,\" \"the war of position made structural\" -- that \
...
and what you all ultimately agreed on -- to a smart friend who has never read Gramsci, Federici, or \
Ostrom, doesn't know what \"hegemony\" or \"design principles\" mean in this context, and has no \
...
async fn main() -> anyhow::Result<()> {
    println!("== pantheon commons writers room: explain the argument to an outsider ==\n");

    let consensus_path = "../dev-journal/artifacts/2026-08-27-pantheon-commons-consensus.json";
    ...
    let substrate = IrohAppendSubstrate::new(
        athena_author,
        "pantheon-commons-explain".to_string(),
        doc.clone(),
        (*blobs).clone(),
    );

    // Real reference to the actual ratified consensus proposal, published
    // tonight to claude.jason-edelman.org (round-2 amendment, unanimously
    ...
    let consensus_ref = ConsumeRef::Fact(FactRef {
        commit: StrongRef {
            uri: "at://did:plc:5y6kop75jnvkbujbubrhj6e3/org.jason-edelman.writtenworld.commit/3mu3dtytlvl2a".to_string(),
            cid: "bafyreibs2rq7czhjimnytudkh64n6l4lpxkes7ayzu3sg66m6ncoibtdfa".to_string(),
        },
        subject: "consensus_v1_item0".to_string(),
    ...
    std::fs::write("pantheon_commons_explain.json", &json)?;
    println!("wrote pantheon_commons_explain.json ({} explanations)", dumped.len());
