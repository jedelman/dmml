...
async fn explain(
    ...
    let user_msg = format!(
        "Below is a synthesis your group (four Olympians) produced after an unusual kind of debate. \
You had already been through a long argument -- seven texts synthesized and ratified, then tested \
against four Black feminist sources that broke several of your own conclusions, then honestly \
reconciled into a document that left one debt explicitly, permanently unresolved. This time you were \
given two new sources specifically about the body: Wilhelm Reich's account of character armor -- \
repression literally braced into muscle tissue -- and sexual repression as the actual mechanism that \
manufactures authoritarian character; and Bill Plotkin's account of the descent to soul, wildness as a \
person's own severed nature rather than somewhere to visit. You used these to ask a very specific \
question: who is even qualified to sit at the table making decisions about the unresolved debt from \
before? You then spent five rounds proposing a series of answers -- a certified spiritual descent, an \
un-armored body visibly demonstrating itself, a 'grammar of deeds' judged only by action, even ecstasy \
itself -- and broke every single one, including catching that a Nazi rally technically satisfies the \
definition of 'ecstasy' your own group had just proposed as the answer. Your group coined shorthand for \
this mid-debate -- terms like 'character armor,' 'the criterion-craving,' 'a mechanism without a \
procedure' -- that mean nothing to someone who wasn't in the room.\n\n\
=== THE SYNTHESIS ===\n{synthesis}\n=== END ===\n\n\
Explain the WHOLE thing -- what question you were actually trying to answer, why each proposed answer \
failed, and what your group concluded once every answer had failed -- to a smart friend with no \
background in any of this. This is a genuinely strange thing to have to explain plainly: your group's \
real conclusion is that repeatedly failing to find an answer WAS the answer, and that conclusion has to \
land as insight, not as a shrug or a dodge. Rules: (1) do not use any of your group's coined shorthand \
...
stranger who reads only your explanation understands both what was tried and why the failure itself was \
the real finding. Three to five short paragraphs. No tool call needed -- just write the explanation \
directly.",
    );
...
async fn main() -> anyhow::Result<()> {
    println!("== pantheon commons embodiment writers room: explain the self-critique ==\n");

    let consensus_path = "../dev-journal/artifacts/2026-08-27-pantheon-commons-embodiment-consensus.json";
    ...
    let substrate = IrohAppendSubstrate::new(
        athena_author,
        "pantheon-commons-embodiment-explain".to_string(),
        doc.clone(),
        (*blobs).clone(),
    );
    ...
    let consensus_ref = ConsumeRef::Fact(FactRef {
        commit: StrongRef {
            uri: "at://did:plc:5y6kop75jnvkbujbubrhj6e3/org.jason-edelman.writtenworld.commit/3mu3q72mqc22s".to_string(),
            cid: "bafyreiemhn4tixx5o47dzkn547peflm3xmlocivcwx7urg4guyf5qhmp6e".to_string(),
        },
        subject: "embodiment_consensus_v0_item0".to_string(),
    ...
    std::fs::write("pantheon_commons_embodiment_explain.json", &json)?;
    println!("wrote pantheon_commons_embodiment_explain.json ({} explanations)", dumped.len());
