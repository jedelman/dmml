...
async fn explain(
    ...
    let user_msg = format!(
        "Below is a synthesis your group (four Olympians) reached after a long debate about seven \
...
(cooperation as nature's dominant law), and Murray Bookchin's The Ecology of Freedom (hierarchy as \
prior to and deeper than class). Your group coined its own vocabulary mid-debate -- terms like \"the \
line of flight,\" \"circulation as the organ of un-learning,\" \"the shipping lane,\" \"subsidized by \
the hunted\" -- that mean nothing to someone who wasn't in the room.\n\n\
=== THE SYNTHESIS ===\n{synthesis}\n=== END ===\n\n\
Explain the WHOLE argument -- what real question it's actually about, what the core disagreement was, \
and what you all ultimately agreed on -- to a smart friend who has never read any of these seven \
authors, doesn't know what \"hegemony\" or \"design principles\" mean in this context, and has no \
political theory background at all. Rules: (1) do not use any of your group's coined shorthand \
...
async fn main() -> anyhow::Result<()> {
    println!("== pantheon commons7 writers room: explain the argument to an outsider ==\n");

    let consensus_path = "../dev-journal/artifacts/2026-08-27-pantheon-commons7-consensus.json";
    ...
    let substrate = IrohAppendSubstrate::new(
        athena_author,
        "pantheon-commons7-explain".to_string(),
        doc.clone(),
        (*blobs).clone(),
    );
    ...
    let consensus_ref = ConsumeRef::Fact(FactRef {
        commit: StrongRef {
            uri: "at://did:plc:5y6kop75jnvkbujbubrhj6e3/org.jason-edelman.writtenworld.commit/3mu3lwmft5m2j".to_string(),
            cid: "bafyreidhatv5ortuxbr4hv4ordpgxqj5zckqshr6oklc7xrljoy7ptqglu".to_string(),
        },
        subject: "consensus7_v1_item0".to_string(),
    ...
    std::fs::write("pantheon_commons7_explain.json", &json)?;
    println!("wrote pantheon_commons7_explain.json ({} explanations)", dumped.len());
