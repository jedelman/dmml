...
**The evidentiary consequence of the meta-agent claim, made concrete by a real failure.** If coordination genuinely lives in the shared, modified substrate rather than in any participant's persistence, a strong consequence follows for what counts as evidence at all: a participant's own account of what it did is not that substrate, and must never be treated as if it were. A small, real incident from a different subsystem than the one above makes this precise rather than aspirational. A tool-calling de-prose agent (`compliance-endurance/deprose_agent.py`, built to let a model draft, check, and commit DMML content through real function calls rather than a scripted retry loop) was run against `openrouter/free`, a router that assigns a different underlying free model to each call in the conversation — the same rotating-participant, shared-history structure as the worktree agents above, at a finer grain. One such run: the model's final message reported "committed blacksmithSituation (15 facts + 7 predicates), capturing the merchant's triple-price squeeze, the closed quarry's local-supply impact, the father's warnings..." — a fluent, specific, entirely false account. No `commit` tool call had been made that round; the output directory was, and remained, empty. The harness never has to adjudicate this claim, because it was never designed to source truth from it: `committed` is counted exclusively from real tool invocations and the files they actually write, and `commit` itself re-runs the identical parse/self-declaration/whole-tree-consistency checks `check` does rather than trusting the model's own judgment that a draft is ready. The model's narration and the world's actual state simply diverged, harmlessly, because only one of the two was ever wired up to be authoritative. Generalized, this is the same discipline the whole-tree gate (Section 10 above) and the schema-conformance harness (Section 9) already apply to *content* — never take a fact on an agent's say-so, check it against the deterministic core — extended to a second, easy-to-miss target: an agent's report of its *own actions*, which is exactly as unverified as any other unchecked claim until something outside the agent's own output confirms it. DMML's committed graph is built to be that confirmation; a transcript of what an agent says it did is not, and this section's evidence is that the gap between the two is not hypothetical.
...
## 11. Truth as commons: protocol against platform, and the limits of the extension

Section 10 establishes the meta-agent claim on functional grounds — perceive/infer/act, occupied by a substrate no participant reduces to. This section, at Jason Edelman's explicit initiative and in his own words, extends the frame beyond functional description into political theory: is what Section 10's substrate produces usefully understood as a commons?

Hardt and Negri's own definition of the common is not primarily material. In *Commonwealth*, they name it as "the common wealth of the material world... and more significantly those results of social production that are necessary for social interaction and further production, such as **knowledges, languages, codes, information, affects**."<sup>[verified 2026-09-03, Hardt and Negri 2009, p. viii, via Allmer's page-cited summary]</sup> A checkable grammar for representing and producing shared world-content sits inside that category directly, not by analogy stretched to fit.

The immediate objection is enclosure: this project's reference implementation, `dmml-hs`, and the repositories it lives in are ordinarily, unremarkably private. Hardt and Negri's own account names exactly this as the threat — "current capitalist accumulation expropriates and destroys the common" (p. 132). The answer is structural, not rhetorical: DMML is a **protocol**, and `dmml-hs` a **reference implementation** of it, a distinction this project's own design language already enforces on independent grounds (`written-world/SPEC.md`'s "A/B razor" — a rule counts as DMML only if it survives as a pass/fail fixture; if it can't, it isn't protocol, it's implementation detail). What's common, on this reading, is not any checkout of the code but the grammar and its checkable semantics — the same sense in which no one's private mail server owns SMTP. Masnick's protocol-vs-platform argument names the mechanism precisely: a platform "locks users in," where a protocol "push\[es] the power and decision making out to the ends of the network."<sup>[verified 2026-09-03, Masnick 2019, Knight First Amendment Institute]</sup> A privately-held implementation doesn't enclose a protocol any more than one ISP's servers enclose the internet.

**On whether this requires treating a model call as a political subject — it doesn't, and the correction is worth stating precisely rather than leaving the reader to guess which version of the claim is intended.** The first draft of this section risked exactly that misreading. Jason's own correction: the claim is not that DMML's dispatch models are subjects in Hardt and Negri's sense, but that they function as **singularities within a collective subjectivity** — and this is, on inspection, the more textually faithful use of their apparatus, not a softened one. The multitude itself is defined this way in their own vocabulary, independent of anything this paper introduces: "composed of innumerable internal differences that can never be reduced to a unity or a single identity... a multiplicity of all these singular differences... many-colored, like Joseph's magical coat" (*Multitude*, 2004, p. xiii–xv). And their account of what composes the common gives singularity, not subjecthood, the load-bearing role: "the production of subjectivity and the encounter of singularities, which compose new assemblages and constitute new forms of the common" (*Commonwealth*, p. 186). Read this way, the claim is not "`kimi-k2.5` wants something" — Section 10 has already refused that, and this section doesn't walk it back — but that the *heterogeneity itself*, each model's difference from the last preserved rather than homogenized into one persona, is what the encounter-and-composition structure requires, and DMML's own rotating-participant, shared-history design (Section 10's worktree agents; the confabulation incident's rotating `openrouter/free` calls) already has that shape by construction, not by aspiration.

**Where the extension still, honestly, doesn't close the gap.** Singularity in Hardt and Negri's own broader frame — and in the Deleuze and Guattari apparatus it draws on, the companion paper's own register — is not merely statistical difference; it carries an expressive dimension, tied to their account of desire and production that this paper has consistently declined to claim for DMML's participants. The difference between `kimi-k2.5` and a Gemma-family free-tier model is a difference of architecture and training, confirmed real and load-bearing for the collective-subjectivity structure above, but it is not established, here or anywhere in this paper, as a difference of *desire* or *perspective* in the sense the source theory means by singularity at full strength. The honest position is: the structural pattern — differences preserved, not homogenized, composing something none of them individually authors — is real and demonstrated; whether that pattern is sufficient for "collective subjectivity" in the fuller sense Hardt and Negri intend, absent the expressive/desiring dimension, is a further philosophical question this paper raises and does not resolve.
